...
def pinwheel_data(num_classes=3, n=10000, radial_std=.3, tangential_std=.05, rate=0.25, norm_data=None):
...
def test_twod():
    ...
    negatives = pinwheel_data(num_classes=4, n=1000*4)#+3.0
    negative_density = make_density_map(negatives, grid_config)

    print('Exemplars', exemplars.shape, exemplars.dtype)
    print(exemplars[:2])
    print('Negatives', negatives.shape, negatives.dtype)
    print(negatives[:2])
    exit()
    ...
    with tf.Session():
        ...
        for i in range(plot_itrs):
            ...
            plt.imshow(np.c_[predictions_to_heatmap(probs, grid_config), negative_density],
                       cmap='afmhot')
            plt.colorbar()
            plt.savefig(fname='data/itr_%s_%d.png' % (ex.name, i))
            plt.clf()
